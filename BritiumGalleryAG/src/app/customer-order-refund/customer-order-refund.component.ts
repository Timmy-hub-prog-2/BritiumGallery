...
@Component({
  ...
  standalone: false,
  templateUrl: './customer-order-refund.component.html',
  styleUrl: './customer-order-refund.component.css'
})
export class CustomerOrderRefundComponent implements OnInit {
  orderId!: number;
  refunds: Refund[] = [];
  loading = false;
  error = '';
  ...
  getStatusCount(status: string): number {
    return this.refunds.filter((refund) => refund.status === status).length
  }
